...
const trustedHelperPath = fileURLToPath(
  new URL("../../native/macos/bin/focus_paste", import.meta.url),
);
...
/** Test-only child boundary; production always invokes the fixed target-built helper. */
export interface MacosChildRunner {
...
class ExecFileMacosChildRunner implements MacosChildRunner {
...
/**
 * Main-only adapter for the bounded macOS helper. It never accepts clipboard
 * text, exposes target identity, uses a shell, or retries a different target.
 */
export class MacosFocusPasteAdapter implements FocusPastePort {
  private active = false;

  public constructor(
    private readonly childRunner: MacosChildRunner = new ExecFileMacosChildRunner(),
...
function encodeRequest(
  ...
  const request = operation === "capture"
    ? { version: protocolVersion, requestId, operation, platform: "darwin" }
    : {
