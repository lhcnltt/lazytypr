...
| Phase 1 Windows hardware validation | On Windows 11 x64, run exactly five focused scenarios: clipboard-only, verified paste only in Notepad, refused/unavailable target copy-only, capture cancellation, and processing cancellation. Verify the overlay stays topmost without activating and every terminal path returns cleanly. |
| Deferred Phase 3 macOS hardware validation | On macOS 13+ arm64, verify TextEdit paste, Accessibility denial/revocation, cancellation, Retina behavior, and repeated lifecycle safety on target hardware. |
